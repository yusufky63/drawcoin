...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.width && body.height) {
      ...
      return await handleImageGenerationWithRetry(
        body as ImageGenerationParams
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          console.log(
            `Rate limit hit (attempt ${attempt + 1}/${MAX_RETRIES}): ${error}`
          );
  ...
  return NextResponse.json(
    {
      ...
      suggestion:
        "Please try again later, the service may be experiencing high demand",
    },
    { status: 429 }
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (!imageResponse.ok) {
        console.error(
          `Image download error: ${imageResponse.status} - ${imageUrl}`
        );
        return NextResponse.json(
          {
            error: `Image download error: ${imageResponse.status}`,
            url: imageUrl,
          },
      ...
      console.log(
        `Image successfully downloaded: ${imageUrl}, size: ${imageBlob.size} bytes`
      );
