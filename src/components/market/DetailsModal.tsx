...
export default function DetailsModal({ token, isOpen, onClose, onTrade }: DetailsModalProps) {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (details && !(details as any).hasError) {
          ...
          setData(null);
          setError((details as any)?.rateLimited ? 'Rate limited. Try again shortly.' : ((details as any)?.error || 'Failed to load details'));
        }
      } catch (e: any) {
        if (!mounted) return;
        setData(null);
      ...
      if (isOpen && address && publicClient && token?.contract_address) {
        try {
          ...
          if (creatorAddress && address.toLowerCase() === creatorAddress.toLowerCase()) {
            ...
            const availableBalance = Math.max(0, parseFloat(tokenBalance) - initialSupply);
            tokenBalance = availableBalance.toFixed(4);
            console.log("Creator detected - filtered 10M initial supply. Available balance:", tokenBalance);
          }
  ...
  const chartUrl = token ? `https://dexscreener.com/base/${token.contract_address}?embed=1&theme=light&trades=0&info=0` : '';
  ...
  const handleQuickPercent = (pct: number) => {
    ...
    const calc = Math.max(0, maxBalance * pct);
    setAmount(calc.toFixed(4));
  };

  const handleQuickEth = (val: string) => setAmount(val);

  const handleTrade = async () => {
    if (!token || !isConnected || !address || !walletClient || !publicClient) return;
    if (!amount || parseFloat(amount) <= 0) return;
    
    // Token addresses on Base
    const USDC_ADDRESS = '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913';
    const ZORA_ADDRESS = await getZORATokenAddress();
    
    const toastId = toast.loading(
      `${tradeType === 'buy' ? 'Buying' : 'Selling'} ${parseFloat(amount).toFixed(4)} ${tradeType === 'buy' ? selectedCurrency : token.symbol}...`,
      { 
        id: "trade-toast",
    ...
    try {
      ...
      // Success toast
      toast.success(
        `🎉 ${tradeType === 'buy' ? 'Buy' : 'Sell'} successful! ${parseFloat(amount).toFixed(4)} ${tradeType === 'buy' ? selectedCurrency : token.symbol} ${tradeType === 'buy' ? 'purchased' : 'sold'}`,
        { 
          id: "trade-toast",
          duration: 4000
        }
      ...
      try { 
        ...
        if (creatorAddress && address.toLowerCase() === creatorAddress.toLowerCase()) {
          const initialSupply = 10000000; // 10M tokens
          const availableBalance = Math.max(0, parseFloat(refreshedBalance) - initialSupply);
      ...
      } else if (e?.message?.includes('Quote failed') || e?.message?.includes('500')) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-2">
      <div className="hand-drawn-card w-full max-w-4xl max-h-[85vh] overflow-hidden flex flex-col" style={{ 
        transform: 'rotate(-0.5deg)',
        maxWidth: '900px',
        maxHeight: '700px'
      }}>
        {/* Sticky Tabs + Close */}
        <div className="sticky top-0 z-10 bg-white border-b-2 border-art-gray-900 px-4 py-3 flex items-center justify-between" style={{ borderStyle: 'dashed' }}>
          <div className="inline-flex items-center gap-1 border-2 border-art-gray-900 rounded-art p-1" style={{ borderRadius: '15px 5px 10px 8px' }}>
            {(['trade','details'] as const).map(t => (
              <button key={t} onClick={() => setActiveTab(t)}
                className={`px-3 py-2 text-xs font-bold transition-all duration-200 ${
                  activeTab===t? 'bg-art-gray-900 text-white' : 'text-art-gray-600 hover:text-art-gray-800'
                }`}
                style={{ 
          ...
          <button onClick={onClose} className="text-art-gray-400 hover:text-art-gray-600 transform rotate-1">
        ...
        {/* Content */}
          ...
          {activeTab === 'trade' ? (
            <>
              <div className="hand-drawn-card p-2 md:p-4" style={{ transform: 'rotate(-0.3deg)' }}>
                {/* Buy/Sell Toggle */}
                ...
                <div className="space-y-3">
                      ...
                      <div className="mt-1 text-xs text-art-gray-500">
                        ≈ ${(() => {
                          if (tradeType === 'buy') {
                            ...
                            return tokenPrice ? (parseFloat(amount) * parseFloat(tokenPrice)).toFixed(2) : '—';
                          }
                        })()} USD
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={(() => {
                        ...
                        return ((parseFloat(amount) / maxBalance) * 100).toString();
                      })()}
                      onChange={(e) => {
                  ...
                  {/* Trade Summary */}
                  {amount && (
                    <div className="bg-art-gray-50 p-2 rounded-art transform rotate-0.3" style={{ borderRadius: '8px 6px 10px 4px' }}>
                      <div className="text-xs text-art-gray-600">
                        {tradeType === 'buy' ? 'Buy' : 'Sell'} {parseFloat(amount).toFixed(4)} {tradeType === 'buy' ? selectedCurrency : token.symbol}
            ...
            <>
              ...
              <div className="hand-drawn-card p-4 mb-4" style={{ transform: 'rotate(0.2deg)' }}>
                <div className="flex items-center gap-4">
                  {/* Token Image Icon */}
                  <div className="overflow-hidden bg-art-gray-50 cursor-pointer flex-shrink-0" style={{ 
              ...
              {/* Token Stats */}
              <div className="hand-drawn-card p-4 mb-4" style={{ transform: 'rotate(0.3deg)' }}>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {/* Price */}
                  <div className="bg-art-gray-50 p-3 rounded-art transform rotate-1" style={{ borderRadius: '12px 4px 8px 6px' }}>
                    <div className="text-art-gray-500 text-xs">Price (USDC)</div>
                  ...
                  {/* Market Cap */}
                  <div className="bg-art-gray-50 p-3 rounded-art transform rotate-0.5" style={{ borderRadius: '10px 5px 15px 8px' }}>
                  ...
                  {/* 24h Volume */}
                  <div className="bg-art-gray-50 p-3 rounded-art transform -rotate-0.5" style={{ borderRadius: '15px 3px 10px 7px' }}>
                  ...
                  {/* Total Volume */}
                  <div className="bg-art-gray-50 p-3 rounded-art transform rotate-1" style={{ borderRadius: '6px 8px 12px 4px' }}>
                    <div className="text-art-gray-500 text-xs">Total Volume</div>
                    <div className="text-art-gray-900 font-bold text-sm">
                      {(() => {
                  ...
                  {/* Holders */}
                  <div className="bg-art-gray-50 p-3 rounded-art transform -rotate-1" style={{ borderRadius: '12px 6px 8px 10px' }}>
                    <div className="text-art-gray-500 text-xs">Holders</div>
                    <div className="text-art-gray-900 font-bold text-sm">{(token as any).uniqueHolders || 0}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="hand-drawn-card p-4 mb-4" style={{ transform: 'rotate(-0.2deg)' }}>
                <div className="grid grid-cols-2 gap-3">
                  {/* Total Supply */}
                  <div className="bg-art-gray-50 p-3 rounded-art transform rotate-0.5" style={{ borderRadius: '10px 5px 15px 8px' }}>
                    <div className="text-art-gray-500 text-xs">Total Supply</div>
                    <div className="text-art-gray-900 font-bold text-sm">
                      {(() => {
                        const supply = (token as any).totalSupply;
                        return supply ? `${(parseFloat(supply) / 1000000).toFixed(2)}M` : '—';
                      })()}
                    </div>
                  </div>
                  {/* Created */}
                  ...
                  {/* Token Address */}
                  <div className="bg-art-gray-50 p-3 rounded-art transform -rotate-1" style={{ borderRadius: '12px 6px 8px 10px' }}>
                    <div className="text-art-gray-500 text-xs">Token Address</div>
              ...
              <div className="hand-drawn-card p-4 mb-4" style={{ transform: 'rotate(-0.2deg)' }}>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-art-gray-500 text-xs mb-1">Creator</div>
                    <div className="text-art-gray-900 font-bold text-sm">
                      {(token as any).creatorProfile?.handle || 'Unknown'}
                    </div>
                    <div className="text-art-gray-500 text-xs font-mono">
                      {(token as any).creatorAddress ? `${(token as any).creatorAddress.slice(0, 6)}...${(token as any).creatorAddress.slice(-4)}` : '—'}
                    </div>
                  </div>
                  <div>
                    <div className="text-art-gray-500 text-xs mb-1">Pool Currency</div>
                    <div className="text-art-gray-900 font-bold text-sm">
                      {(token as any).poolCurrencyToken?.name || 'ZORA'}
                    </div>
                    <div className="text-art-gray-500 text-xs">
                      {(token as any).poolCurrencyToken?.decimals || 18} decimals
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Description */}
              <div className="hand-drawn-card p-4 mb-4" style={{ transform: 'rotate(0.1deg)' }}>
                <div className="text-art-gray-500 text-xs mb-2">Description</div>
              ...
              {/* External Links */}
              <div className="hand-drawn-card p-4" style={{ transform: 'rotate(-0.3deg)' }}>
                <div className="space-y-3">
                  <a 
      ...
      {/* Large Image Modal */}
      {showImageModal && (
        <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-4" onClick={() => setShowImageModal(false)}>
          <div className="relative max-w-[90vw] max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            ...
            {(() => {
              const imageUrl = (token as any).mediaContent?.previewImage?.medium || (token as any).mediaContent?.previewImage?.small || token.image_url;
              return imageUrl ? (
                <img 
                  src={imageUrl} 
                  alt={token.name || 'Token Image'} 
                ...
                <div className="w-96 h-96 flex items-center justify-center text-white bg-gray-800 rounded-lg">
                  <div className="text-center">
                    <svg className="w-16 h-16 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ strokeWidth: 2 }}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
