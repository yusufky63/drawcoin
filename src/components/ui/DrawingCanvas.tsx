...
const DrawingCanvas: React.FC<DrawingCanvasProps> = ({
  ...
  const [currentTool, setCurrentTool] = useState<
    "brush" | "eraser" | "fill" | "line" | "shape" | "eyedropper" | "text"
  >("brush");
  const [currentColor, setCurrentColor] = useState("#000000");
  const [brushSize, setBrushSize] = useState(8);
  const [brushType, setBrushType] = useState<
    "normal" | "soft" | "hard" | "marker"
  >("normal");
  ...
  const startDrawing = (
    ...
    // Check lock state first - allow scrolling if locked
    if (isLocked) return;
    ...
    if (currentTool === "line" || currentTool === "shape") {
      ...
      baseImageDataRef.current = ctx.getImageData(
        0,
        0,
        canvas.width,
        canvas.height
  ...
  const toolsComponent = (
    <div className={`space-y-3 ${toolsVariant === "compact" ? "pt-1" : ""}`}>
      ...
      <div
        className={`flex items-center ${
          toolsVariant === "compact"
            ? "gap-1 overflow-x-auto no-scrollbar py-1"
            : "gap-2 flex-wrap"
        }`}
      >
        <button
          className={`hand-drawn-btn tool-btn text-xs md:text-sm px-2 py-1 md:px-3 md:py-2 ${
            currentTool === "brush" ? "secondary" : ""
          }`}
        ...
        <button
          className={`hand-drawn-btn tool-btn text-xs md:text-sm px-2 py-1 md:px-3 md:py-2 ${
            currentTool === "text" ? "secondary" : ""
          }`}
          onClick={() => setCurrentTool("text")}
          title="Text"
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24">
            <path d="M4 4h16v2H13v14h-2V6H4z" />
          </svg>
          {toolsVariant === "full" && <span className="ml-1">Text</span>}
        </button>
        <button
          className={`hand-drawn-btn tool-btn text-xs md:text-sm px-2 py-1 md:px-3 md:py-2 ${
            currentTool === "eyedropper" ? "secondary" : ""
          }`}
          onClick={() => setCurrentTool("eyedropper")}
          title="Eyedropper"
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24">
            <path d="M14.828 2.586a2 2 0 012.828 0l3.758 3.758a2 2 0 010 2.828l-3.536 3.536-1.414-1.414 2.121-2.121-2.344-2.344-2.121 2.121-1.414-1.414 2.122-2.122-1-1zM4 16l6-6 4 4-6 6H4v-4z" />
          </svg>
      ...
      <div
        className={`flex items-center ${
          toolsVariant === "compact" ? "gap-2 py-1" : "gap-2"
        }`}
      >
        <button
          className={`hand-drawn-btn tool-btn text-xs px-2 py-1 ${
            isLocked ? "secondary" : ""
          }`}
          onClick={() => setIsLocked(!isLocked)}
          title={isLocked ? "Unlock Drawing" : "Lock Drawing (Enable Scroll)"}
        >
          {isLocked ? (
            <svg className="w-4 h-4" viewBox="0 0 24 24">
        ...
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24">
            <path d="M12 5V1L7 6l5 5V7c3.31 0 6 2.69 6 6a6 6 0 01-6 6H6v2h6a8 8 0 000-16z" />
          </svg>
        </button>
        <button
          className="hand-drawn-btn tool-btn text-xs px-2 py-1"
        ...
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24">
            <path d="M12 5V1l5 5-5 5V7C8.69 7 6 9.69 6 13a6 6 0 006 6h6v2h-6a8 8 0 010-16z" />
          </svg>
        </button>
        <button
          className="hand-drawn-btn tool-btn danger text-xs px-2 py-1"
          onClick={() => {
            pushUndoSnapshot();
            clearCanvas();
          }}
        ...
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24">
            <path d="M5 20h14v-2H5v2zm7-18L8.5 7H11v6h2V7h2.5L12 2z" />
          </svg>
        </button>
      </div>
            ...
            {currentTool === "brush" && (
              <div className="space-y-2">
                ...
                <div className="flex gap-1">
                  <button
                    className={`hand-drawn-btn text-xs px-2 py-1 ${
                      brushType === "normal" ? "secondary" : ""
                    }`}
                  ...
                  >
                    <svg className="w-3 h-3" viewBox="0 0 24 24">
                      <path
                        d="M7 14c-1.66 0-3 1.34-3 3 0 1.31-1.16 2-2 2 .92 1.22 2.49 2 4 2 2.21 0 4-1.79 4-4 0-1.66-1.34-3-3-3z"
                        style={{ opacity: 0.7 }}
                      />
                    </svg>
                  </button>
            ...
            {currentTool === "shape" && (
              <div className="space-y-2">
                ...
                <div className="flex gap-1">
                  <button
                    className={`hand-drawn-btn text-xs px-2 py-1 ${
                      currentShape === "rectangle" ? "secondary" : ""
                    }`}
                  ...
                  <button
                    className={`hand-drawn-btn text-xs px-2 py-1 ${
                      currentShape === "triangle" ? "secondary" : ""
                    }`}
                    onClick={() => setCurrentShape("triangle")}
                    title="Triangle"
                  >
                    <svg className="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M12 2l9 18H3l9-18z" />
                    </svg>
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-2">
              ...
              {showCustomColor && (
                <div className="flex items-center gap-2 p-2 bg-white rounded-lg border border-art-gray-200">
                  <input
                    ...
                    className="w-8 h-6 rounded border border-art-gray-300"
                  />
                  <span className="text-xs font-mono text-art-gray-600">
                    {currentColor}
                  </span>
                </div>
              )}
              <div className="grid grid-cols-6 md:grid-cols-12 gap-1">
                {colors.map((color) => (
                  <button
                    key={color}
                    className={`color-picker relative ${
                      currentColor === color
