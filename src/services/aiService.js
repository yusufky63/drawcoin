...
const API_ENDPOINTS = {
  ...
  GEMINI_IMAGEN3_FAST:
    "https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-fast:generateImages",
};
...
const getApiKey = () => {
  ...
  if (!apiKey) {
    ...
    throw new Error(
      "API key is required. Please set TOGETHER_API_KEY in your environment variables."
    );
  }
  return apiKey;
};
...
const generateGeminiImageStream = async (userPrompt) => {
  ...
  const dataUrl = `data:${firstImageData.mimeType || "image/png"};base64,${
    firstImageData.data
  }`;
  return dataUrl;
};
...
const generateImageWithTogetherAI = async (userPrompt) => {
  ...
  const response = await fetch(API_ENDPOINTS.TOGETHER_IMAGE, {
    ...
    body: JSON.stringify({
      ...
      response_format: "url",
      negative_prompt:
        "photorealistic, photograph, 3d render, digital art, computer generated, low quality, blurry, distorted, text, words, letters, numbers",
    }),
  });
...
export const generateImageWithAI = async (userPrompt) => {
  ...
  const apiProviders = [
    {
      name: "Together.ai",
      generator: () => generateImageWithTogetherAI(userPrompt),
    },

    // Prefer Gemini via SDK streaming (fast + high quality)
    {
      name: "GeminiSDK",
      generator: () => generateGeminiImageStream(userPrompt),
    },
...
const generateImageWithGemini = async (userPrompt) => {
  ...
  const body = {
    ...
    negativePrompt:
      "photorealistic, photograph, 3d render, digital art, computer generated, low quality, blurry, distorted, text, words, letters, numbers, watermark",
    imageFormat: "png",
    width: 1024,
    height: 1024,
  ...
  const resp = await fetch(
    `${API_ENDPOINTS.GEMINI_IMAGEN3_FAST}?key=${GEMINI_API_KEY}`,
    {
      method: "POST",
