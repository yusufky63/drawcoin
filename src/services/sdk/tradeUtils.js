...
export async function getZORATokenAddress() {
  try {
    ...
    console.warn(
      "Failed to get ZORA token address from SDK, using fallback:",
      error
    );
  }
...
export async function validateCoinForTrade(coinAddress) {
  try {
    ...
    if (
      ...
      console.warn(
        "API validation failed, allowing trade to proceed - trade will handle validation"
      );
      return true;
    }
    return false;
